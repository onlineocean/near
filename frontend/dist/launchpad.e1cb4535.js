...
function handleTabletChange(e) {
    if (e.matches) {
        ...
        for(i = 0; not.length > i; i++)not[i].onclick = function() {
            let current = mod[0];
            if (current) {
                setTimeout(function() {
...
for(i = 0; modalItem.length > i; i++)modalItem[i].onclick = function() {
    let currentActive = borderColor[0];
    if (currentActive) currentActive.classList.remove("borderColor");
    if (currentActive !== this) this.classList.add("borderColor");
};
...
//# sourceMappingURL=launchpad.e1cb4535.js.map
