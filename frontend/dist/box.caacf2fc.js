/* Create Card */ const cardItems = document.querySelectorAll(".create__group-item"), backLink = document.querySelectorAll(".create__inner-link");
...
const inputs = document.querySelectorAll(".line-chek"), btn = document.querySelector(".btn--non"), inputsArr = Array.from(inputs), price = document.querySelector(".line__price"), nameOfWork = document.querySelector(".name__work"), nameInput = document.querySelector(".line__name"), procent = document.querySelector(".procent"), priceCount = document.querySelector(".jsCount"), multipleInput = document.querySelector(".multip-input");
function limitedValue(nameOfValue, count) {
    if (nameOfValue.textContent.length > count) nameOfValue.textContent = nameOfValue.textContent.slice(0, count) + "...";
}
inputs.forEach((item)=>{
    ...
    item.addEventListener("input", ()=>{
        ...
        if (item.value == "") statusMessage.style.color = "#e85b67";
        else statusMessage.style.color = "transparent";
        limitedValue(nameOfWork, 16);
        if (inputsArr.some((elem)=>elem.value == "") || !isNaN(price.value) == false || multipleInput.value < 2) btn.disabled = true;
        ...
        } else price.value = price.value.replace(/[\D0]/, "");
...
multipleInput.addEventListener("input", ()=>{
    ...
    else multipleInput.value = multipleInput.value.replace(/[\D0]/, "");
    if (inputsArr.some((elem)=>elem.value == "") || !isNaN(price.value) == false || multipleInput.value < 2) btn.disabled = true;
    else btn.disabled = false;
    if (multipleInput.value < 2) statusMessageCopies.style.color = "#e85b67";
    else statusMessageCopies.style.color = "transparent";
    multipleInput.insertAdjacentElement("afterend", statusMessageCopies);
});
if (inputsArr.some((elem)=>elem.value == "")) btn.disabled = true;
else btn.disabled = false;
const multipleBlock = document.querySelector(".multiple");
if (localStorage.getItem("nameOfCard") == "Multiple") multipleBlock.classList.add("mulblock");
else multipleBlock.classList.remove("mulblock");
/* Tab */ let tab = document.querySelectorAll(".tab-header"), tabContent = document.querySelectorAll(".tab-content");
function hideTab() {
    tab.forEach((item)=>{
...
//# sourceMappingURL=box.caacf2fc.js.map
